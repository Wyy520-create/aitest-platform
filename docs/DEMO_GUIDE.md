...
> 本文档回答五个问题：①这个项目是什么、3 分钟怎么讲明白；②从零部署运行，每个环节的细节（含 Windows、Docker 启动关闭）；③面试现场怎么用 cpolar 公网演示；④技术面高频问题怎么答；⑤"印象最深的 bug"怎么答。
...
"aitest-platform 是一个智能接口测试平台。测开简历项目有个通病——只有测试代码、没有真实的被测系统。所以我先写了一个电商系统 mini-mall，**故意埋了 7 个真实缺陷**；平台对它跑 45 条自动化接口用例，**7 个缺陷全部命中**；还带 AI 用例生成（生成后经人工评审门禁才入库）、RAG 知识库问答（自实现检索 + 拒答保护）。docker 一键起全栈，CI 三条作业 88 秒自动回归。您 clone 下来 10 分钟就能跑通。"

### 3 分钟面试版（四段式：痛点 → 架构 → 演示 → 技术决策）

**第 1 分钟 · 为什么做 + 是什么：**
"测开岗简历项目最容易被质疑的一点是：测试代码写得再好，被测系统在哪？所以我从零写了一个带缺陷的电商系统 mini-mall——越权、并发超卖、金额精度、边界静默错误，都是真实线上常见的事故类型。然后围绕它搭了完整的测试平台：Pytest 分层引擎、Vue3 + FastAPI 平台前后端、AI 用例生成和 RAG 知识问答两个 AI 模块，全部 docker 化，CI 自动回归。"

**第 2 分钟 · 数字 + 演示路径：**
"四个数字概括：45 条自动化用例命中 7 个缺陷；RAG 评测集 21 条、MRR 0.972 进 CI 防回归；三条 CI 作业 88 秒跑完；平台 5 分钟能演示完——执行中心一键跑套件看 7 个失败用例的缺陷定位、AI 工场生成用例草稿走人工评审、RAG 问答演示正常回答和拒答。"

**第 3 分钟 · 三个技术决策（为什么这么做）：**
"一、执行层数据驱动，被测系统地址是唯一环境变量 `SUT_BASE_URL`——换系统不改代码；二、AI 生成不直接入库，必须人工评审——因为 LLM 会幻觉，**度量 AI 而不只是使用 AI**；三、RAG 拒答阈值用'得分率'而不是绝对分——BM25 绝对分和查询长度正相关，短查询会被误杀，这是跑数据实测出来的，不是拍的。每个决策的踩坑过程都写在了代码注释和 git 历史里。"
...
| 系统 | 做法 |
|---|---|
| Windows 10/11 | 官网下载 **Docker Desktop for Windows** 安装包，双击安装（会自动启用 WSL2，装完重启电脑）。内存建议 4GB+ |
...
# ② 让当前用户免 sudo 用 docker（不配的话每条 docker 命令都要加 sudo，compose 会报权限错误）
sudo usermod -aG docker $USER

# ③ 重新登录终端（关闭终端重开，或注销重登）——用户组改动必须重新登录才生效！

# ④ 验证安装成功（看到 "Hello from Docker!" 即成功）
docker run hello-world
```

**启动 / 关闭 Docker 本身**：Windows/macOS——Docker Desktop 就是个普通软件，双击启动、菜单里 Quit 关闭（建议设置里勾"开机自启"）；Linux——`sudo systemctl start docker` / `sudo systemctl stop docker`。日常只是暂停/继续跑项目的话不用动 Docker 本身，用下面 1.7 的 compose 命令管项目容器就够了。
...
### 1.3 Windows 部署（PowerShell，与上面等价）

```powershell
git clone https://github.com/Wyy520-create/aitest-platform.git
cd aitest-platform
Copy-Item .env.example .env      # PowerShell 的 cp 等价命令
docker compose up -d --build
# 浏览器打开 http://localhost:3000
```

**Windows 三个经典坑**：
1. **Docker Desktop 没启动**：`docker compose` 报错先打开 Docker Desktop，等左下角鲸鱼图标变绿；
2. **`.env` 变成了 `.env.txt`**：记事本保存时"保存类型"选"所有文件"，文件名就是 `.env`；
3. **老版本 Win10 提示启用 WSL2**：点 Docker Desktop 的提示链接按向导操作即可（需要一次重启）。

> 面试官机器上没有 Git？浏览器打开 `https://github.com/Wyy520-create/aitest-platform` → 绿色 Code 按钮 → Download ZIP → 解压进目录，从第 ② 步继续。
...
| 端口被占（3000/8000/8100） | 改 `docker-compose.yml` 的 `ports:` 映射如 `"3001:80"`，再 `up -d` |
...
**换电脑要不要重新配？要。** Authtoken 跟账号走，但配置写在本机文件里（Linux/macOS 是 `~/.cpolar/cpolar.yml`），新电脑上必须重新执行一次第 ③ 步。已注册的老用户换新机器，直接做 ②③ 两步即可。

Windows/macOS 客户端：官网下载安装后，同样先在终端/PowerShell 执行一次 `cpolar authtoken xxx`（客户端安装只是多了图形界面，隧道还是命令行开）。
...
**兜底方案**（面试官死活不想注册 cpolar 时）：已装 Node 的话 `npx localtunnel --port 3000`，或有 cloudflared 的话 `cloudflared tunnel --url http://localhost:3000`，两条都无需注册。
...
刻意的教学与演示取舍：一个 index.html 零构建、零 node_modules，面试官打开文件就能读懂全链路（模板→响应式→API 调用），docker 镜像也极简（nginx + 单文件）。代价是踩了 in-DOM 模板与 Element Plus 的兼容坑（见 bug 故事四），已修复并注释。
